...
//    console.log(xhr);
  ...
  getMP3: function(search,callback){
    ...
    this.getSearchResults(search, function(songinfo){
        ...
        gs.getStreamKey(songinfo.id,function(song){
            //item.setProperty(ns+"streamKey",song.streamKey)
            //item.setProperty(ns+"streamServer",song.streamServer);
            console.log("key:"+song.streamKey);
            console.log("server:"+song.streamServer);
      ...
      try{
        if (req.readyState==4 && req.status==200){
          ...
          for (var i in songs){
            ...
            var cont = songcallback.apply(gs,[songdata]);
            if (cont==false)
                break;
          }
        }
  ...
  getCommunicationToken: function(callback,arg1,arg2){
    ...
    req.onreadystatechange = function(ev){
      ...
      try{status=req.status;}catch(e){}
        try{
        if (req.readyState==4){
          if (status==200 && !hasloaded && req.responseText&&req.responseText!=""){
  ...
  _getPostData: function(method,parameters){
    ...
    var postdata={
      ...
      {
        "session":gs.session,
        "token":gs.computeToken(method,gs.currentToken),
        "uuid":gs.uuid,
        "clientRevision":gs.clientRevision,
        "client":"gslite",
  ...
  _sendPost: function(method,parameters,callback,service){
    ...
    try{
    var gs = this;
    var poststr = this._getPostData(method,parameters);
    var post = JSON.stringify(poststr);
    var req=new xhr.XMLHttpRequest();
    ...
    req.onreadystatechange = function(ev){
      ...
      try{
        if (status==200&&!hasloaded){
          try{
            callback.apply(gs,[req.responseText]);
            hasloaded=true;
          }catch(e){}
        }
      }catch(e){}
